...
import by.algorithm.alpha.system.visuals.mainmenu.main.MainScreen;
...
import by.algorithm.alpha.system.events.TickEvent;
...
import by.algorithm.alpha.api.modules.impl.player.ItemCooldown;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public void setDefaultMinecraftTitle() {
        ...
        if (selfDestruct.unhooked) {
            ...
            this.mainWindow.setWindowTitle("Solth ~ Build v1.0 (Dev)");
        }
    }

    private String getWindowTitle() {
        return("Solth ~ Build v1.0 (Dev)");
    }
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        Initclass.getInstance().getEventBus().post(tickEvent);
                ...
                for (Hand hand : Hand.values()) {
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY -> {
                                ModuleReg functionRegistry = Initclass.getInstance().getFunctionRegistry();
                                AttackAura killAura = functionRegistry.getKillAura();
    ...
    public void runTick() {
        ...
        if (this.world != null) {
            Initclass.getInstance().getEventBus().post(player.tick);
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        Initclass.getInstance().getEventBus().post(new EventChangeWorld());
    }
