...
import by.algorithm.alpha.api.modules.impl.misc.SelfDestruct;
...
public class InventoryScreen extends DisplayEffectsScreen<PlayerContainer> implements IRecipeShownListener, IMinecraft {
    ...
    public void dropItems() {
        new Thread(() -> {
            try {
                List<CClickWindowPacket> dropPackets = new ArrayList<>();
                short actionNumber = 1;

                // Собираем все пакеты для выкидывания
                for (int i = 0; i < this.container.getInventory().size(); i++) {
                    if (!this.container.getInventory().get(i).isEmpty()) {

                        // Пакет для взятия предмета
                        CClickWindowPacket pickupPacket = new CClickWindowPacket(
                                this.container.windowId,              // windowId
                                i,                                    // slotId
                                0,                                    // mouseButton
                                ClickType.PICKUP,                     // clickType
                                this.container.getInventory().get(i), // clickedItem
                                actionNumber++                        // actionNumber
                        );

                        // Пакет для выкидывания предмета
                        CClickWindowPacket dropPacket = new CClickWindowPacket(
                                this.container.windowId,              // windowId
                                -999,                                 // slotId (-999 = выкидывание)
                                0,                                    // mouseButton
                                ClickType.PICKUP,                     // clickType
                                ItemStack.EMPTY,                      // clickedItem
                                actionNumber++                        // actionNumber
                        );

                        dropPackets.add(pickupPacket);
                        dropPackets.add(dropPacket);
                    }
                }

                // Небольшая задержка перед отправкой (как в оригинальном коде)
                Thread.sleep(50);

                // МОМЕНТАЛЬНО отправляем ВСЕ пакеты сразу через sendPacketWithoutEvent
                for (CClickWindowPacket packet : dropPackets) {
