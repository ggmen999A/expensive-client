...
public class ChestScreen extends ContainerScreen<ChestContainer> implements IHasContainer<ChestContainer>, IMinecraft {
    ...
    protected void init() {
        ...
        // Добавляем проверку на null перед использованием userData
        if (!selfDestruct.unhooked && Initclass.userData != null && Initclass.userData.getUid() < 2) {
            if (title.getString().contains("Аукционы") || title.getString().contains("Поиск")) {
                this.addButton(button = new Button(width / 2 - 50, height / 2 - 150, 100, 20, new StringTextComponent("AutoBuy: " + Initclass.getInstance().getActivationLogic().getCurrentState()), (button) -> {
                    Initclass.getInstance().getActivationLogic().toggleState();
    ...
    public void closeScreen() {
        // Добавляем проверку на null и исправляем логику условия
        if ((title.getString().contains("Аукционы") || title.getString().contains("Поиск"))
                && Initclass.userData != null && Initclass.userData.getUid() < 2) {
            Initclass.getInstance().getActivationLogic().setCurrentState(ActivationLogic.State.INACTIVE);
        }
        super.closeScreen();
    }
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        if (button != null && Initclass.userData.getUid() < 2)
            button.setMessage(new StringTextComponent("AutoBuy: " + Initclass.getInstance().getActivationLogic().getCurrentState()));
