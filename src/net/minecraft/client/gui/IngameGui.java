...
import by.algorithm.alpha.api.modules.impl.misc.BetterMinecraft;
import by.algorithm.alpha.api.modules.impl.render.Crosshair;
...
public class IngameGui extends AbstractGui {
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        ...
        if (eventDisplay.getType() == EventDisplay.Type.PRE) {
            Initclass.getInstance().getEventBus().post(eventDisplay);
        }
        ...
        if (!this.mc.gameSettings.hideGUI) {
            ...
            Initclass.getInstance().getEventBus().post(eventCancelOverlay);
    ...
    protected void renderHotbar(float partialTicks, MatrixStack matrixStack) {
        ...
        boolean isBetterHotbar = betterMinecraft.isState() && betterMinecraft.betterHotbar.get();

        if (isBetterHotbar) {
            renderBetterHotbar(partialTicks, matrixStack, player);
        } else {
            renderVanillaHotbar(partialTicks, matrixStack, player);
        }
    }

    private void renderBetterHotbar(float partialTicks, MatrixStack matrixStack, PlayerEntity player) {
        int centerX = this.scaledWidth / 2;
        int hotbarY = this.scaledHeight - 22;
        int slotWidth = 18;
        int slotHeight = 18;
        int totalWidth = 10 * slotWidth + 10;
        DisplayUtils.drawRoundedRect(
                centerX - totalWidth/2 - 5,
                hotbarY - 2,
                totalWidth + 10,
                24,
                5,
        ...
        for (int i = 0; i < 9; ++i) {
            int x = centerX - totalWidth/2 + i * slotWidth + (i > 0 ? 1 : 0);
            int y = hotbarY + 1;
            ...
            this.renderHotbarItem(x + 3, y + 3, partialTicks, player, player.inventory.mainInventory.get(i));
        }
        ItemStack offhandItem = player.getHeldItemOffhand();
        if (!offhandItem.isEmpty()) {
            HandSide handside = player.getPrimaryHand().opposite();
            int offhandX = handside == HandSide.LEFT ?
                    centerX - totalWidth/2 - 26 :
                    centerX + totalWidth/2 + 6;
            ...
            this.renderHotbarItem(offhandX + 3, hotbarY + 4, partialTicks, player, offhandItem);
        }
        if (this.mc.gameSettings.attackIndicator == AttackIndicatorStatus.HOTBAR) {
            float attackStrength = this.mc.player.getCooledAttackStrength(0.0F);
            if (attackStrength < 1.0F) {
                int indicatorY = this.scaledHeight - 20;
                int indicatorX = centerX + totalWidth/2 + 10;

                if (player.getPrimaryHand().opposite() == HandSide.RIGHT) {
                    indicatorX = centerX - totalWidth/2 - 26;
                }
                DisplayUtils.drawRoundedRect(
                        indicatorX,
                        indicatorY,
                ...
                int progressHeight = (int)(attackStrength * 18);
                DisplayUtils.drawRoundedRect(
                        indicatorX,
                        indicatorY + 18 - progressHeight,
                        18,
                        progressHeight,
                        3,
                        0x8000FF00
    ...
    private void renderVanillaHotbar(float partialTicks, MatrixStack matrixStack, PlayerEntity player) {
        ...
        this.blit(matrixStack, i - 91 - 1 + player.inventory.currentItem * 20, this.scaledHeight - 22 - 1, 0, 22, 24, 22);
        ...
        for (int i1 = 0; i1 < 9; ++i1) {
            ...
            this.renderHotbarItem(j1, k1, partialTicks, player, player.inventory.mainInventory.get(i1));
        }
        ...
        if (!itemstack.isEmpty()) {
            ...
            if (handside == HandSide.LEFT) {
                this.renderHotbarItem(i - 91 - 26, i2, partialTicks, player, itemstack);
            } else {
                this.renderHotbarItem(i + 91 + 10, i2, partialTicks, player, itemstack);
            }
            CustomItems.setRenderOffHand(false);
        }
