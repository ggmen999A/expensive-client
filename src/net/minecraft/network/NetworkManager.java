...
public class NetworkManager extends SimpleChannelInboundHandler<IPacket<?>> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, IPacket<?> p_channelRead0_2_) throws Exception {
        ...
        if (this.channel.isOpen()) {
            ...
            Initclass.getInstance().getEventBus().post(receive);
            if (receive.isCancel()) {
                receive.open();
                return;
            }
    ...
    public void sendPacket(IPacket<?> packetIn, @Nullable GenericFutureListener<? extends Future<? super Void>> p_2010582) {
        ...
        if (!event.isCancel()) {
            if (this.isChannelOpen()) {
                this.flushOutboundQueue();
                this.dispatchPacket(event.getPacket(), p_2010582);
            } else {
                this.outboundPacketsQueue.add(new QueuedPacket(event.getPacket(), p_2010582));
            }
        }
    }
        ...
        new Bootstrap().group(lazyvalue.getValue()).handler(new ChannelInitializer<Channel>() {
            ...
            protected void initChannel(Channel channel) {
                ...
                if (channel instanceof SocketChannel) {
                   ...
                   if (ViaLoadingBase.getInstance().getTargetVersion().getVersion() != Initclass.getInstance().getViaMCP().getNATIVE_VERSION()) {
