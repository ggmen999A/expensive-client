...
import by.algorithm.alpha.api.modules.impl.misc.AntiPush;
import by.algorithm.alpha.api.modules.impl.render.SwingAnimation;
...
public abstract class LivingEntity extends Entity {
    ...
    public boolean onLivingFall(float distance, float damageMultiplier) {
        ...
        if (i > 0) {
            ...
            if (this instanceof ClientPlayerEntity) {
                Initclass.getInstance().getEventBus()
    ...
    protected void jump() {
        ...
        if (this instanceof ClientPlayerEntity entity) {
            Initclass.getInstance().getEventBus().post(jumpEvent);
        }
    ...
    public void travel(Vector3d travelVector) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                ModuleReg functionRegistry = Initclass.getInstance().getFunctionRegistry();
                AttackAura attackAura = functionRegistry.getKillAura();
                if (attackAura.isState() && attackAura.getOptions().get(1).get() && this instanceof ClientPlayerEntity) {
                    vector3d1 = this.getVectorForRotation(attackAura.rotateVector.y, attackAura.rotateVector.x);
                }

                if (attackAura.isState() && attackAura.getOptions().get(1).get() && this instanceof ClientPlayerEntity) {
                    f = attackAura.rotateVector.y * ((float)Math.PI / 180F);
                }
                ...
                f1 = (float)((double)f1 * (double)f1 * Math.min(1.0F, d4 / 0.4));
                vector3d = this.getMotion().add(0.0F, d0 * ((double)-1.0F + (double)f1 * (double)0.75F), 0.0F);
                if (vector3d.y < (double)0.0F && d1 > (double)0.0F) {
                    double d5 = vector3d.y * -0.1 * (double)f1;
                    vector3d = vector3d.add(vector3d1.x * d5 / d1, d5, vector3d1.z * d5 / d1);
                }

                if (f < 0.0F && d1 > (double)0.0F) {
                    double d9 = d3 * (double)(-MathHelper.sin(f)) * 0.04;
                    vector3d = vector3d.add(-vector3d1.x * d9 / d1, d9 * 3.2, -vector3d1.z * d9 / d1);
                }

                if (d1 > (double)0.0F) {
                    vector3d = vector3d.add((vector3d1.x / d1 * d3 - vector3d.x) * 0.1, (double)0.0F, (vector3d1.z / d1 * d3 - vector3d.z) * 0.1);
                }
                ...
                if (this.collidedHorizontally && !this.world.isRemote) {
                    ...
                    double d6 = d3 - d10;
                    float f2 = (float)(d6 * (double)10.0F - (double)3.0F);
                    if (f2 > 0.0F) {
