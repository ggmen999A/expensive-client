        ...
        if (this.isAttachedToEntity())
        {
            ...
            if (this.boostedEntity != null)
            {
                if (this.boostedEntity.isElytraFlying())
                {
                    ...
                    // Проверка на ElytraBoost модуль
                    if (Initclass.getInstance().getFunctionRegistry().getElytraBoost().isState()) {
                        if (Initclass.getInstance().getFunctionRegistry().getElytraBoost().isAntiTargetEnabled) {
                            speed = (Float)Initclass.getInstance().getFunctionRegistry().getElytraBoost().speedantitarget.get();
                        } else if (AttackAura.getTarget() != null) { // Теперь работает как статический вызов
                            speed = (Float)Initclass.getInstance().getFunctionRegistry().getElytraBoost().speedtoTarget.get();
                        } else {
                            speed = (Float)Initclass.getInstance().getFunctionRegistry().getElytraBoost().speedwithTarget.get();
                        }
                    }

                    // Проверка на специальный питч
                    if (this.boostedEntity.rotationPitch == -30.0F && !Initclass.getInstance().getFunctionRegistry().getElytraBoost().isAntiTargetEnabled) {
                        speed = defaultSpeed;
                    }

                    // Интеграция с KillAura для коррекции движения
                    if (Initclass.getInstance().getFunctionRegistry().getKillAura().isState()
                            && (Boolean)Initclass.getInstance().getFunctionRegistry().getKillAura().getOptions().getValueByName("Коррекция движения").get()
                            && this.boostedEntity instanceof ClientPlayerEntity) {

                        Vector3d vector3d = this.getVectorForRotation(
                                Initclass.getInstance().getFunctionRegistry().getKillAura().rotateVector.y,
                                Initclass.getInstance().getFunctionRegistry().getKillAura().rotateVector.x
                        );
                        Vector3d vector3d1 = this.boostedEntity.getMotion();
                        this.boostedEntity.setMotion(vector3d1.add(
                                vector3d.x * 0.1 + (vector3d.x * (double)speed - vector3d1.x) * (double)0.5F,
                                vector3d.y * 0.1 + (vector3d.y * (double)speed - vector3d1.y) * (double)0.5F,
                                vector3d.z * 0.1 + (vector3d.z * (double)speed - vector3d1.z) * (double)0.5F
                        ));
                    } else {
