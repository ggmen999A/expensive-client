package by.algorithm.alpha.system.visuals.autobuy;
...
public class AutoBuyConfig implements IMinecraft {
    ...
    public static void updateFile() {
        ...
        for (AutoBuy autoBuy : Initclass.getInstance().getAutoBuyHandler().items) {
    ...
    private void readBuyConfig() throws FileNotFoundException {
        ...
        if (jsonObject.has("autoBuys")) {
            for (JsonElement element : jsonObject.get("autoBuys").getAsJsonArray()) {
                ...
                Initclass.getInstance().getAutoBuyHandler().items.add(new AutoBuy(item, price,autoBuyObject.get("count").getAsInt(), enchantments,
