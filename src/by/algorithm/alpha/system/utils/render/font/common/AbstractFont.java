package by.algorithm.alpha.system.utils.render.font.common;
...
import by.algorithm.alpha.system.utils.render.font.TextureHelper;
import by.algorithm.alpha.system.utils.render.font.Wrapper;
...
public abstract class AbstractFont implements Wrapper {
    ...
    // Константа для пути к шрифтам (для обратной совместимости)
    private static final String FONT_DIR = "/assets/minecraft/expensive/font/";
    ...
    public static final Font getFont(String fileName, int style, int size) {
        ...
        try {
            font = Font.createFont(Font.TRUETYPE_FONT, Objects.requireNonNull(AbstractFont.class.getResourceAsStream(path)))
                    .deriveFont(style, size);
        } catch (FontFormatException | IOException e) {
            System.err.println("Ошибка загрузки шрифта: " + path);
            e.printStackTrace();
    ...
    public static final Font getFontWindows(String fileName, int style, int size) {
        ...
        try {
            ...
            System.err.println("Ошибка загрузки системного шрифта: " + fileName);
            e.printStackTrace();
