package by.algorithm.alpha.system.utils.render.font.styled;
...
import by.algorithm.alpha.system.utils.render.font.common.AbstractFont;
...
public final class StyledFont {
    ...
    public StyledFont(ResourceLocation fontResource, int size, float stretching, float spacing, float lifting, boolean antialiasing, Lang lang) {
        int[] codes = lang.getCharCodes();
        char[] chars = new char[(codes[1] - codes[0] + codes[3] - codes[2])];
        ...
        Font baseFont = loadFontFromResource(fontResource, size);
        this.regular = new GlyphPage(baseFont, chars, stretching, spacing, lifting, antialiasing);
    }

    private Font loadFontFromResource(ResourceLocation fontResource, int size) {
        try {
            InputStream fontStream = null;

            try {
                fontStream = Minecraft.getInstance().getResourceManager().getResource(fontResource).getInputStream();
            } catch (Exception e) {
                String classPath = "/assets/" + fontResource.getNamespace() + "/" + fontResource.getPath();
                fontStream = StyledFont.class.getResourceAsStream(classPath);

                if (fontStream == null) {
                    classPath = "/assets/minecraft/" + fontResource.getPath();
                    fontStream = StyledFont.class.getResourceAsStream(classPath);
                }
            }

            if (fontStream == null) {
                throw new RuntimeException("Cannot find font: " + fontResource);
            }

            Font baseFont = Font.createFont(Font.TRUETYPE_FONT, fontStream);
            Font derivedFont = baseFont.deriveFont(Font.PLAIN, (float) size);

            fontStream.close();
            return derivedFont;

        } catch (Exception e) {
            System.err.println("Ошибка загрузки шрифта из ResourceLocation: " + fontResource);
            e.printStackTrace();
